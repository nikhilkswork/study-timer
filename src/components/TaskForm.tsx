...
function formReducer(state: FormState, action: FormAction): FormState {
  switch (action.type) {
    ...
    case 'TOGGLE_INFINITE_LOOP':
      const nextInfiniteLoop = !state.isInfiniteLoop;
      return {
        ...state,
        isInfiniteLoop: nextInfiniteLoop,
        isInfinite: false,
        hours: state.isInfinite ? 0 : state.hours || 0,
        minutes: state.isInfinite ? 25 : state.minutes || 25,
        error: '',
      };
    case 'SET_PRESET':
      return {
...
export function TaskForm({ onClose, editId, editTitle, editDuration, editIsInfinite, editIsInfiniteLoop }: TaskFormProps) {
  const [state, dispatch] = useReducer(formReducer, {
    ...
    isInfiniteLoop: editIsInfiniteLoop || false,
    error: '',
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (state.isInfinite) {
      if (editId) {
        editTask(editId, trimmedTitle, 0, true, false);
      } else {
        addTask(trimmedTitle, 0, true, false);
      }
    } else {
      ...
      if (editId) {
        editTask(editId, trimmedTitle, totalMinutes, false, state.isInfiniteLoop);
      } else {
        addTask(trimmedTitle, totalMinutes, false, state.isInfiniteLoop);
      }
    }
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-3">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl border transition-all text-[10px] font-bold uppercase tracking-wider
              ${
                state.isInfinite
          ...
          >
            {state.isInfinite ? '∞ Infinite: ON' : '∞ Infinite Session'}
          </button>

          <button
            type="button"
            onClick={() => dispatch({ type: 'TOGGLE_INFINITE_LOOP' })}
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl border transition-all text-[10px] font-bold uppercase tracking-wider
              ${
                state.isInfiniteLoop
                  ? 'bg-[var(--foreground)] text-[var(--background)] border-transparent shadow-[0_0_24px_rgba(255,255,255,0.06)]'
                  : 'bg-[var(--input-bg)] border-[var(--border)] text-[var(--muted)] hover:text-[var(--foreground)] hover:bg-[var(--hover)]'
              }`}
          >
            {state.isInfiniteLoop ? '∞ Loop: ON' : '∞ Pomodoro Loop'}
          </button>
        </div>
      </div>
